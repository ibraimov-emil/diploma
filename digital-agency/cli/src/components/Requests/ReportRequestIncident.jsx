...
const ReportRequestIncident = ({ requestId, requestTitle, visible, onClose }) => {
  ...
  const onFinish = (values) => {
    ...
    const incidentData = {
      title: values.title?.trim() || `Проблема с заявкой #${requestId}`,
      ...
      metadata: JSON.stringify({
        ...
        additionalInfo: values.metadata?.trim() || ''
      })
    };
  ...
  return (
    <Modal
      title={<span><WarningOutlined style={{ color: '#ff4d4f', marginRight: '8px' }} />Сообщить о проблеме с заявкой</span>}
      open={visible}
    ...
    >
      <p className="text-gray-500 mb-4">
        Используйте эту форму для сообщения о проблемах, связанных с заявкой #{requestId}.
      </p>
      ...
      <Form
        ...
        onFinish={onFinish}
        initialValues={{
          title: `Проблема с заявкой #${requestId}`,
          severity: 'medium'
        }}
      >
        <Form.Item
        ...
        <Form.Item
          ...
          rules={[{ required: true, message: 'Пожалуйста, опишите проблему с заявкой' }]}
        >
          <TextArea 
            placeholder="Опишите, что именно не работает или какие проблемы у вас возникли с заявкой" 
            rows={4} 
          />
        </Form.Item>
...
export default ReportRequestIncident; 