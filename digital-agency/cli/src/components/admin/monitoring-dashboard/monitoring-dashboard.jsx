...
const MonitoringDashboard = () => {
  ...
  const [uptime, setUptime] = useState(99.5); // Default fallback value
  ...
  const fetchData = async () => {
    ...
    try {
      let uptimeData = 99.4; // Default fallback value
      ...
      try {
        ...
        if (healthResponse.data && healthResponse.data.status) {
          healthData = healthResponse.status;
        }
      } catch (err) {
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} md={4}>
          <Card>
            ...
            <CardContent>
              <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', flexDirection: 'column' }}>
                <Typography variant="h3" component="div" color={healthStatus === 200 ? 'success.main' : 'error.main'}>
                  {healthStatus === 200 ? 'Работает' : 'Недоступен'}
                </Typography>
                <Typography variant="body2" color="text.secondary">
                  {lastHealthCheck 
