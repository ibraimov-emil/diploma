...
async function _start() {
  ...
  _classPrivateFieldGet(_logger, this).log(`CPU profiling is started for process '${process.pid}'.`);
}
async function _stop() {
  try {
    ...
    _classPrivateFieldGet(_logger, this).log(`CPU profiling is stopped for process '${process.pid}'.`);
    ...
    _classPrivateFieldGet(_logger, this).log(`Saved CPU profile to '${path}'.`);
  } finally {
...
function _default(program) {
  program.option('--profiler.signal <signal>', 'Start/stop CPU profiling on <signal>').on('option:profiler.signal', function (signal) {
    ...
    const logger = new _logger2.Logger();
